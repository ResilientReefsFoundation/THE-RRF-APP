...
import { toLocalYYYYMMDD, getSiteName } from './DashboardPage';
...
const OperationalSchedulePage: React.FC<OperationalSchedulePageProps> = ({ 
    ...
    const handleCreateShift = (date: string) => {
        ...
        const newShift: VolunteerShift = {
            ...
            pendingVolunteerIds: [],
            maxCapacity: 12,
            siteId: '' // Explicitly empty so nothing is preselected
        };
        onAddShift(newShift);
    };

    const handleUpdate = (shift: VolunteerShift) => {
        if (isReadOnly) return;
        onUpdateShift({
            ...shift,
            date: toLocalYYYYMMDD(shift.date)
        });
    };
    ...
    const handleWipeMonth = async () => {
        ...
        if (confirm(`PERMANENTLY DELETE ALL CLOUD DATA FOR ${monthName.toUpperCase()}?\n\nThis will trigger an immediate atomic sync. The data will be gone forever.`)) {
    ...
    return (
        <div className="bg-white p-6 sm:p-8 rounded-[3rem] shadow-2xl space-y-8 border-2 border-coral-blue animate-fade-in min-h-[85vh] flex flex-col">
            ...
            <div className="w-full overflow-x-auto custom-scrollbar rounded-[3.5rem] shadow-2xl flex-grow border-4 border-gray-100">
                <div className="grid grid-cols-7 gap-px bg-gray-100 min-w-[800px]">
                    ...
                    {calendarDays.map((day, idx) => {
                        ...
                        const dayShifts = (shifts || []).filter(s => s && toLocalYYYYMMDD(s.date) === dateStr);
            ...
            {/* MISSION PLAN MODAL */}
            {selectedDate && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={handleClosePlan}>
                    <div className="bg-white rounded-[2rem] w-full max-w-2xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto flex flex-col animate-fade-in gap-6" onClick={e => e.stopPropagation()}>
                        <div className="flex justify-between items-center border-b pb-4 shrink-0 sticky top-0 bg-white z-10">
                            <div>
                                <h3 className="text-2xl font-black text-coral-dark uppercase tracking-tighter italic leading-none">TODAYS SHIFTS</h3>
                                <p className="text-sm font-black text-coral-blue uppercase tracking-widest mt-1">{selectedDate}</p>
                            </div>
                            <div className="flex items-center gap-3">
                                {!isReadOnly && (
                                    <button 
                                        onClick={() => handleCreateShift(selectedDate)}
                                        className="bg-coral-blue text-white font-black px-6 py-3 rounded-xl text-[10px] uppercase shadow-md hover:brightness-110 transition-all flex items-center gap-2 border-b-4 border-blue-700 active:scale-95"
                                    >
                                        <PlusCircleIcon className="w-4 h-4" />
                                        Add Shift
                                    </button>
                                )}
                                <button onClick={handleClosePlan} className="p-3 bg-gray-100 rounded-full hover:bg-gray-200 transition-colors shadow-sm"><CloseIcon className="w-5 h-5 text-coral-dark"/></button>
                            </div>
                        </div>

                        <div className="space-y-6 flex-grow">
                            <div className="space-y-6">
                                {shifts.filter(s => toLocalYYYYMMDD(s.date) === selectedDate).map(shift => {
                                    // REFINED Logic: Custom is only active if explicitly set to 'CUSTOM_SITE_MARKER' or if a custom name exists with no site ID
                                    ...
                                    return (
                                        <div key={shift.id} className="p-6 border-4 border-coral-blue/10 rounded-[2rem] space-y-6 bg-white relative group shadow-inner">
                                            {!isReadOnly && <button onClick={() => { if(confirm('DELETE MISSION?')) onDeleteShift(shift.id); }} className="absolute top-4 right-4 text-gray-200 hover:text-red-500 transition-colors"><TrashIcon className="w-5 h-5" /></button>}
                                            
                                            {/* Deployment Site Section */}
                                            <div className="space-y-4">
                                                <p className="text-[10px] font-black text-coral-blue uppercase tracking-[0.3em] border-l-4 border-coral-blue pl-3">Deployment Site</p>
                                                <div className="flex flex-wrap gap-2">
                                                    {sites.filter(s => !s.isArchived).map(s => {
                                                        ...
                                                        return (
                                                            <button 
                                                                ...
                                                                disabled={isReadOnly} 
                                                                className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all border-2 shadow-sm ${isSelected ? 'bg-coral-blue border-blue-700 text-white shadow-md scale-105' : 'bg-white border-gray-100 text-gray-400 hover:border-blue-200 hover:text-coral-blue'}`}
                                                            >
                                                                {s.name}
                                                            </button>
                                                        );
                                                    ...
                                                    <button 
                                                        ...
                                                        disabled={isReadOnly} 
                                                        className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all border-2 shadow-sm ${isOtherSelected ? 'bg-orange-600 border-orange-700 text-white shadow-md scale-105' : 'bg-white border-gray-100 text-gray-400 hover:border-blue-200 hover:text-orange-600'}`}
                                                    >
                                                        Custom Site...
                                                    </button>
                                                </div>
                                                
                                                {/* Custom Site Text Input */}
                                                {isOtherSelected && (
                                                    <div className="mt-2 animate-fade-in">
                                                        <input 
                                                            ...
                                                            className="w-full p-4 border-2 border-orange-100 rounded-xl bg-white font-bold text-coral-dark text-lg outline-none focus:border-orange-500 transition-all shadow-inner"
                                                            disabled={isReadOnly}
                                                        />
                                                    </div>
                                                )}
                                            ...
                                            <div className="space-y-4">
                                                <p className="text-[10px] font-black text-blue-600 uppercase tracking-[0.3em] border-l-4 border-blue-600 pl-3">Assigned Staff</p>
                                                <div className="flex flex-wrap gap-2">
                                                    {staffMembers.map(s => {
                                                        ...
                                                        return (
                                                            <button 
                                                                ...
                                                                disabled={isReadOnly} 
                                                                className={`px-4 py-2.5 rounded-xl text-[10px] font-black uppercase transition-all border-2 shadow-sm ${isAssigned ? 'bg-blue-600 border-blue-700 text-white shadow-md scale-105' : 'bg-white border-gray-100 text-gray-400 hover:border-blue-200 hover:text-blue-600'}`}
                                                            >
                                                                {s.name}
                                                            </button>
                                                        );
                                            ...
                                            {/* Field Volunteers Section */}
                                            <div className="space-y-4">
                                                <p className="text-[10px] font-black text-green-600 uppercase tracking-[0.3em] border-l-4 border-green-600 pl-3">Field Volunteers</p>
                                                <div className="space-y-2">
                                                    {(shift.assignedVolunteerIds || []).map(vid => {
                                                        const v = volunteers.find(vol => String(vol.id).trim() === String(vid).trim());
                                                        return (
                                                            <div key={vid} className="flex items-center gap-3 w-full bg-green-50 text-green-700 px-4 py-3 rounded-2xl border-2 border-green-100 text-xs font-black uppercase justify-between group shadow-sm">
                                                                {v?.name || 'Volunteer'}
                                                                <button onClick={() => { if (isReadOnly) return; const currentVols = Array.isArray(shift.assignedVolunteerIds) ? shift.assignedVolunteerIds : []; handleUpdate({...shift, assignedVolunteerIds: currentVols.filter(id => String(id).trim() !== String(vid).trim())}); }} className="opacity-0 group-hover:opacity-100 text-red-500 p-1 hover:bg-red-100 rounded-lg transition-all font-black text-lg">&times;</button>
                                                            </div>
                                                        );
                                                    })}
                                                </div>
                                                
                                                {/* CRITICAL: Volunteer Selection Dropdown */}
                                                {!isReadOnly && (
                                                    <div className="relative mt-2">
                                                        <select
                                                            ...
                                                            value=""
                                                            className="w-full p-3 border-2 border-green-100 rounded-xl bg-white text-green-700 font-black uppercase text-[10px] outline-none focus:border-green-500 transition-all appearance-none cursor-pointer hover:border-green-300 shadow-sm"
                                                        >
                                                        ...
                                                        <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-green-600 bg-green-100 p-1 rounded-full">
                                                            <PlusCircleIcon className="w-4 h-4" />
                                                        </div>
                                                    </div>
                                                )}
                        ...
                        <div className="flex flex-col gap-4 sticky bottom-0 bg-white pt-4 shrink-0 border-t mt-2">
                            <button onClick={handleClosePlan} className="w-full py-5 font-black rounded-[1.5rem] uppercase tracking-[0.3em] text-xs shadow-xl active:scale-95 transition-all flex items-center justify-center gap-6 border-b-[8px] bg-coral-dark text-white border-black hover:brightness-110">
                                <CheckCircleIcon className="w-8 h-8" /> SAVE AND CLOSE
                            </button>
                        </div>
                    </div>
                </div>
